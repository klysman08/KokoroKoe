...
- Task: P5-016 - Proactive recent-insight generation
- Date: 2026-08-22
- Outcome: added one exact-main `generate_recent_insights` command that reads only the last twelve finalized segments of the addressed Session, requests exactly the frozen preset's own insight types through the existing prompt/budget/transport/validation/repair/retry boundaries, and renders at most eight validated typed insights transiently in the active Session card without persisting any generated text. The stale README was replaced with the real current state and regenerable build residue was removed.
- Verification: locked Rust checks passed rustfmt, Clippy with warnings denied, 295 ordinary tests, 14 ignored explicit gates, and 4 capability tests; locked frontend checks passed 37 Vitest files/171 tests, the Vite build, and repository policy. A loopback test proved no provider secret canary, workspace path, or out-of-window transcript text reaches the wire and that ZDR/data-collection-denied routing is set on every request. Audits, the 160-package license inventory, the locked no-bundle x64 release, and a responsive five-second launch smoke passed. The only new webview authorization is `allow-generate-recent-insights`; the existing transitive `nanoid <3.3.18` finding remains and no dependency, lockfile, generic permission, persistence, event, extra-window, or audio boundary was added.
- Memory: [P5-016 checkpoint](docs/project-memory.md#p5-016---proactive-recent-insight-generation)
