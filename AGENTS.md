...
- Task: P6-002 - Transcript window opacity, always-on-top, and compact mode
- Date: 2026-08-23
- Outcome: gave the detached transcript window its background opacity, always-on-top, and compact mode while keeping that window command-free. A Rust service owns the appearance, the main window changes it through exact-main commands, and Rust publishes it as an event the window already had permission to receive — on page load and on every accepted change. Opacity dims only the window background behind fully opaque text, and is clamped to a readable floor and quantized, so the window can never be made invisible.
- Verification: locked Rust checks passed rustfmt, Clippy with warnings denied, 315 ordinary tests, 14 ignored explicit gates, and 6 capability tests; locked frontend checks passed 44 Vitest files/194 tests, the Vite build, and repository policy. Tests prove out-of-range, non-finite, and unknown-field appearance payloads are rejected, that values quantize, that the controls invoke nothing while collapsed, and that the detached window applies a received appearance and ignores an out-of-range one without invoking a command. Audits, the 160-package license inventory, the locked no-bundle x64 release, and a responsive five-second launch smoke passed. The only new webview authorizations are `allow-get-transcript-window-appearance` and `allow-set-transcript-window-appearance`; no dependency or lockfile changed. An incidental fix gave the test suite an in-memory Web Storage, since the jsdom build provides none and `AppShell.test.tsx` crashed without it.
- Limitation to close next: the translucent rendering was verified by contract and unit tests only. A human must confirm on a real desktop that text stays readable at the opacity floor before R-010 can record its readable-text half.
- Memory: [P6-002 checkpoint](docs/project-memory.md#p6-002---transcript-window-opacity-always-on-top-and-compact-mode)
