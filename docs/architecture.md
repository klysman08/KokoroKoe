...
P6-006 adds the second secondary window. The insights window is display-only for the same reason the transcript window is: after a generation succeeds, Rust publishes the batch as a `session-insights` event, so the window renders insights without holding a command and cannot ask for a generation of its own. Generation stays where it was — an explicit click in the main window — because insights must never be produced implicitly.

The published batch is deliberately narrower than the command's reply. It carries the insights and the project and Session they belong to, and nothing else: cost, budget, attempt counts, and repair accounting stay with the main window, which asked for the work and is the only surface that can act on those numbers. A failed publish never fails the generation, since the main window already has the result and the detached window is an optional second view.

P6-011 adds the two section 9.2 actions that write to the record of the meeting: marking a segment important, and correcting one. Both are recorded as **events**, not edits. The journal is append-only, so a correction is a new `SegmentCorrection` record naming a segment; the `FinalizedTranscriptSegment` it refers to is never touched. Keeping the original recoverable is therefore a property of the storage model rather than a rule somebody has to remember.

Replay folds those events onto the segments they name, last write wins, and produces a `ReplayedSegment` carrying the journaled transcription and the correction side by side. Everything downstream reads through it: the reading view, search, and the summary pipeline all take the effective text, so a correction is what the user sees, searches, and summarizes, while `original_text()` still returns what was transcribed.

`transcript.md` keeps both readings. A corrected segment renders the correction as its body and the transcription beneath it as a blockquote, because Markdown is the portable source of truth — somebody holding only the folder, or only that one file, must still be able to see what was actually said. An annotation is written into the segment's metadata comment **only when true**: `render_transcript` is also what verifies an existing document, so emitting the new keys unconditionally would make every transcript written before this feature fail verification and fall into recovery. The frozen golden fixture is unchanged, which is the proof.

An annotation always follows the segment it names, so replay treats a reference to an unknown segment as corruption rather than an ordering to tolerate — and the append refuses it outright, so an orphan never reaches the file. A correction obeys the same text bounds a transcription does, since it is rendered in exactly the same place.

P6-012 makes that recoverability readable. Replay answers what a segment reads as now by collapsing its corrections; `SessionJournal::segment_history` reads the same records and declines to fold them, which is the only way to recover a wording that was corrected and then corrected again — the document carries the current reading and the transcription, and nothing in between. The transcription it reports is the journaled record itself rather than a reconstruction. The history is bounded at 64 revisions and drops the **oldest** when it overflows, so the transcription and the current reading always survive, and a truncated history says so. The read is served by one exact-main command, `get_transcript_segment_history`; it writes nothing, and the app asks for it only when the reader opens the disclosure. Restoring an earlier wording fills the correction box rather than saving, because putting an old wording back is itself a correction and goes through the same append that any other one does.

P6-009 adds the Manifest section 9.2 per-segment actions the same way, and the same test applies: what does each one require? Copying is view-local. "Explain", "suggest a response", and "summarize" are prefilled questions through the one already-verified `ask_manual_question` path rather than three new prompt specs, so the answer stays inside the schema that path validates and no new command, capability, or prompt version appears.

Prefilling and not sending is the deliberate part. This application sends meeting text to a third party, so a one-click label must not be the thing that transmits: the preset opens the question box with text already in it, and the user reads it, edits it, or abandons it before anything leaves the machine. The panel is keyed by segment and preset so choosing a different one replaces the text rather than leaving a stale question in front of a new label.

Copying is offered on transcript segments but not on search hits, because a hit renders a snippet with match markers rather than the segment's own text; copying it would quote something the transcript does not say.

P6-008 adds the per-insight actions from Manifest section 9.3, and they were chosen for what they do **not** require. Pinning, dismissing, and copying are all view-local, so the insights window keeps its event-only capability and gains no command. Generating an alternative version is the one section 9.3 action that cannot be view-local, and it is deliberately left out rather than made the reason to widen a display-only capability.

The reading state lives in one pure reducer rather than in the component, because every operation moves the same three things — which cards exist, which one is being read, and what the user has already rejected — and getting that wrong shows up as a blank card or a lost pin. Pinned cards survive the next batch, which is what pinning means; everything else is replaced, because insights describe the last few minutes and not a growing record. A new batch lands on the first newly arrived card, so a pin set several batches ago never hides what the user just asked for.

Dismissal is what makes the Manifest's "avoid repeatedly generating the same insight" real from the user's side. The model has no memory of what it already said, so the same observation can arrive again; the window remembers dismissed insights by type, title, and content — the same identity the contract already uses to reject duplicates inside one batch — and drops repeats before they reach the screen. That set is bounded, and overflow can only let an insight reappear, never hide one the user did not dismiss. Pins are bounded too, and the ceiling refuses a new pin rather than silently discarding an old one.

P6-007 keys all of that state by window, which is what Manifest section 10 requires: both windows must have independent opacity, always-on-top, compact mode, geometry, quick-hide, and click-through. The window service now holds one runtime per window and takes the window as an argument everywhere; `window_state` was already keyed by label, so nothing had to migrate. The frontend renders the same controls component twice, once per window, and every reply and event names the window it describes so a listener can ignore the ones that are not its own — and the adapter rejects a reply about the wrong window rather than rendering one window's state under the other's controls.

Two windows also mean two hotkeys. `RegisterHotKey` scopes registrations to the registering thread, so both live on the one message-loop thread and are told apart by a per-window id; the loop tracks which ids it currently holds and releases exactly one on a rebind. Each window ships a distinct default binding, because a shared one would mean the second registration is always refused and the user would see a taken-shortcut warning they did not cause.

ADR 0009 records the one property this changed: the window-management commands now take a window argument. It is a closed enum of the windows Rust owns, not a label, so an unknown value fails to deserialize before the command body runs and the frontend still cannot name, address, or size an arbitrary webview.

ADR 0008 fixes how that constraint is applied. Every window has its own capability file scoped to exactly that window and receives only what its job requires. The `main` capability holds the product command surface; the `transcript` and `insights` capabilities hold event subscription only, because a display-only window needs no command. Both are checked by one shared assertion, so a third window cannot pass by being tested more loosely than the first two. Every command keeps the exact-`main` authorization rule, so the capability layer and the Rust authorization layer reject a secondary window independently and neither is load-bearing alone. Rust owns secondary window creation: label, URL, title, and initial size are module constants, the opening command takes no argument, and no window holds `core:webview:allow-create-webview-window`. Only the main window is declared in the configuration and created at startup; opening an existing secondary window focuses it instead of creating a duplicate, and closing an absent one succeeds.
