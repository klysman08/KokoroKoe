...
- P3-008 is not wired to the live capture/VAD scheduler, model catalog, Tauri commands/events, UI, or persistence. It uses the same executable in an internal worker mode but does not validate an installer layout, signed binary, CPU-only host, AMD/Intel Vulkan, repeated worker restart policy, device loss, parent crash, CPU-inference cancellation, or memory/RSS behavior. Anonymous pipes are local and private to the spawned child; protocol encryption/authentication is intentionally absent because no cross-user or network transport exists.
