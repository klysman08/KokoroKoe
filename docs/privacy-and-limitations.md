...
P3-010 adds a Rust-owned HTTPS model-download boundary but does not expose it through a product command or UI. URLs, revisions, filenames, lengths, and hashes are fixed by the two-entry curated catalog; callers cannot supply a URL or filesystem path. Partial and final model files contain third-party weights under the app-local model root, never user audio or transcript content. Ordinary tests contact no network; the exact gate only reads existing external P3-004 weights. Model IDs and aggregate progress may later cross a typed event boundary, but paths, raw HTTP failures, weight bytes, and resume metadata must not.
