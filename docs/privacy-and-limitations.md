...
- Generated insights are transient. They are never written to Markdown, SQLite, or logs, so they are not recoverable once the surfaces showing them are closed; the finalized transcript they were derived from remains saved locally. A generated batch is emitted locally as an event so the detached insights window can display it, which does not leave the machine and is not retained by Rust: opening that window after a generation shows nothing until the next one.
- The detached insights window shows model-generated text about the meeting on screen in its own frame. It has no controls of its own yet — no opacity, always-on-top, compact mode, quick-hide, shortcut, or click-through — so consider who can see the display before opening it in a shared or screen-shared environment.
...
- The correction history is read from the local recovery journal and nothing else. Opening it sends no network request, writes nothing, and reads only the session you are looking at; wordings you superseded are shown to you because they are already on your disk, not because they were kept anywhere else. Only the 64 most recent corrections are displayed, and older ones stay in the journal unread.
...
- Each show/hide shortcut is a system-wide hotkey: while KokoroKoe is running it reserves that key combination from every application. Each requires Ctrl, Alt, or Win so it cannot capture ordinary typing, each can be turned off entirely, and KokoroKoe registers at most one combination per detached window — two in total. It records no other keystroke and contains no key logging.
...
- Lowering opacity dims the window background only; text stays fully opaque and readable. Opacity cannot be reduced below a readable floor, so the window can never be made invisible and unrecoverable. A translucent window still shows its text over whatever is behind it, which can make that text visible in a screen recording of another application.
- Each window's appearance, position, size, and shortcut are remembered separately between runs in the local non-secret settings database, keyed by window. This records only window geometry, appearance, and the chosen key combination: no transcript text, insight text, session identity, or project content is stored with it. Deleting the local settings database resets it.
...
- Click-through can be switched on per window so that window stops accepting the mouse and clicks reach whatever is behind it. While it is on the window cannot be moved, resized, or closed, so the switch lives in the main window and three rules keep pointer control recoverable: it is never written to disk and so never survives a restart or a crash, it cannot be switched on while the main window is absent, and every window's click-through is switched off automatically when the main window closes. A window shows an explicit indicator while it is on, because a window passing clicks through otherwise looks exactly like a frozen one.
