...
Begin `P3-004`, a bounded local Whisper runtime and Tiny/Base throughput decision prototype. First resolve R-006 by recording explicit approval for `whisper-rs`'s Unlicense or choosing a minimal MIT `whisper.cpp` C adapter; then use one licensed/generated deterministic fixture to measure CPU real-time factor and final latency for multilingual Tiny and Base, select the default only if R-004 passes, and prove model-load/inference failures stay source-local and bounded. Keep model downloading/catalog UI, live transcript events, partial scheduling, retained audio, persistence, OpenRouter, and product UI out of P3-004; GPU fallback/isolation remains R-005 unless explicitly included as a separately frozen gate.
