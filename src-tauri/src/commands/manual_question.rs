...
use crate::{
    domain::{AppError, AskManualQuestionRequest, CommandError, ManualQuestionResponse},
    llm::ManualQuestionService,
    logging,
    security::authorize_main_window,
};

#[tauri::command]
pub(crate) async fn ask_manual_question<R: tauri::Runtime>(
    webview_window: WebviewWindow<R>,
    state: State<'_, ManualQuestionService>,
    request: AskManualQuestionRequest,
) -> Result<ManualQuestionResponse, CommandError> {
    let service =
        authorized(webview_window.label(), || state.inner().clone()).map_err(record_error)?;
    run_blocking(move || service.ask(request))
        .await
...
async fn run_blocking<T: Send + 'static>(
    ...
    tauri::async_runtime::spawn_blocking(operation)
        .await
        .map_err(|_| AppError::manual_question_error("manual_question_worker_failed"))?
}
...
mod tests {
    ...
    #[test]
    fn manual_question_authorizes_before_service_access() {
