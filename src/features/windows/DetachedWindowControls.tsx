...
export function DetachedWindowControls({
  ...
  // Folded by default: two windows of controls would otherwise fill the whole
  // sidebar and push everything else out of reach.
  ...
  const settingsId = `${window}-window-settings`

  return (
    <section aria-label={title} className="mb-3 rounded-xl border p-3">
      <div className="flex items-center justify-between gap-2">
        <p className="truncate text-xs font-medium">{title}</p>
        {/* Popping out is the common action, so it stays one click away even
            while the rest of the controls are folded. */}
        <div className="flex shrink-0 items-center">
          ...
          <Button
            ...
            aria-label={`${expanded ? "Hide" : "Show"} ${title.toLowerCase()} settings`}
        ...
        <div id={settingsId}>
          <div className="mt-3 flex gap-2">
            <Button
              ...
              className={cn(
                "flex-1",
                appearance?.alwaysOnTop && "border-primary",
              )}
              disabled={pending || !appearance}
              onClick={() =>
                void run(() =>
                  update({ alwaysOnTop: !appearance?.alwaysOnTop }),
                )
              }
              size="sm"
              variant="outline"
          ...
          <label className="mt-2 flex items-start gap-2 text-xs">
            <input
              ...
              onChange={(event) =>
                void run(() =>
                  updateShortcut({ enabled: event.target.checked }),
                )
              }
              type="checkbox"
            />
          ...
          {shortcut?.enabled && !shortcut.registered && (
            <p className="text-xs text-amber-700" role="status">
              Windows did not accept {shortcut.binding}. Another application —
              or this app's other window — is probably using it; try a different
              combination.
            </p>
          )}
          <p className="text-muted-foreground mt-1 text-xs">
            Needs Ctrl, Alt, or Win, and each window needs its own combination.
            The same shortcut hides and shows the window, and{" "}
            <strong>Pop out</strong> always brings it back.
          </p>
          ...
          <p className="text-muted-foreground mt-1 text-xs">
            The window stops accepting the mouse entirely — you cannot move,
            resize, or close it while this is on. Turn it off here; it also
            switches off if this window closes, and never survives a restart.
          </p>
        </div>
      )}
    </section>
